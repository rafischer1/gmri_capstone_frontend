...
const FloodDataList = ({ floodData }) => {
  const ulStyle = {
    ...
    marginBottom: '2%',
    fontFamily: 'Aleo',
    borderBottom: '1px dashed rgb(123, 189, 228)'
  };

  return floodData === undefined || floodData === null ? (
    <Spinner className='spinner' name='wave' color='teal' />
  ) : (
    floodData.map(el => {
      let elDate = FormatDate(el.createdat);
      return (
        <thead key={el.id} style={ulStyle}>
          <tr>
            <td>{elDate}</td>
